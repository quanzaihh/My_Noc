...
`define Noc_Node_Active     {(`Noc_X_Size*`Noc_Y_Size){1'b1}}     // 使能哪些节点
`define Noc_Pass_Node       {(`Noc_X_Size*`Noc_Y_Size){1'b0}}     // 对于不使能的节点，是否连接
`define Noc_Link_Reg        {(`Noc_X_Size*`Noc_Y_Size){1'b0}}     // 对于不使能但连接的节点，是否打拍       
...
`define Axi_Addr_Point          (`Axi_Pack_Order_Point - `Axi_ADDR)
`define Axi_Len_Point           (`Axi_Addr_Point - `Axi_LEN_Bit)
`define Noc_Point_E             (`Axi_Len_Point - `Noc_Head_Bit)
`define Less_Byte               (`Noc_Data_Width - `Noc_Head_Bit * 2 - `Noc_ID_X_Width * 2 - `Noc_ID_Y_Width * 2 - `Axi_TYPE_Bit - `Axi_PACK_ORDER_Bit - `Axi_LEN_Bit - `Axi_ADDR)
